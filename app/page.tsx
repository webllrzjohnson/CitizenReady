...
const LANDING_CHAPTERS = [
  { n: 1, name: 'Rights and Responsibilities', description: 'Canadian citizenship rights and responsibilities', questions: '90+' },
  { n: 2, name: 'Who Are Canadians', description: 'Canadian identity, diversity, and Indigenous peoples', questions: '90+' },
  { n: 3, name: "Canada's History", description: 'Historical events and milestones in Canadian history', questions: '90+' },
  { n: 4, name: 'Government', description: 'Canadian government structure and institutions', questions: '90+' },
  { n: 5, name: 'Federal Elections', description: 'Electoral system and voting process', questions: '90+' },
  { n: 6, name: 'Justice System', description: 'Canadian laws and justice system', questions: '90+' },
  { n: 7, name: 'Canadian Symbols', description: 'National symbols and heritage', questions: '90+' },
  { n: 8, name: "Canada's Regions", description: 'Geographic regions and provincial characteristics', questions: '90+' },
  { n: 9, name: "Canada's Economy", description: 'Economic sectors and industries', questions: '90+' },
  { n: 10, name: 'Modern Canada', description: 'Contemporary Canadian society and culture', questions: '90+' },
  { n: 11, name: 'Applying for Citizenship', description: 'Citizenship application process and requirements', questions: '90+' },
]

const TESTIMONIALS = [
  {
    quote: 'I passed on my first try. The timed mock exams felt exactly like the real thing — I walked into the test centre confident.',
    name: 'Priya K.',
    province: 'Ontario',
  },
  {
    quote: 'Chapter practice made it easy to study after work. Seeing my weak topics helped me focus where I needed it most.',
    name: 'Marcus T.',
    ...
    <div className="flex flex-col">
          ...
          <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-8">
            <div className="space-y-8">
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap">
                ...
                <Button size="lg" variant="outline" className="h-12 rounded-full border-white bg-transparent px-8 text-base text-white hover:bg-white/10 hover:text-white" asChild>
                  <Link href="/signup">Sign up to save progress</Link>
                </Button>
                <Button size="lg" variant="ghost" className="h-12 rounded-full px-8 text-base text-white hover:bg-white/10 hover:text-white" asChild>
            ...
            <div className="flex justify-center lg:justify-end">
              <div className="flex h-48 w-48 select-none items-center justify-center rounded-full bg-[#243558] text-8xl shadow-card md:h-64 md:w-64 md:text-9xl" aria-hidden>
                🍁
              </div>
            </div>
          </div>
      ...
      <section id="how-to-prepare" className="bg-white py-16 md:py-24">
        <div className="container mx-auto px-4">
          <h2 className="mb-12 text-center text-3xl font-bold text-brand-navy md:text-4xl">How to Prepare</h2>
          <div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-3 md:gap-8">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-[#424242]">Study each chapter of Discover Canada with topic-specific questions</p>
            </div>
            <div className="flex flex-col items-center text-center">
              <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-brand-red-light text-brand-red">
              ...
              <p className="text-[#424242]">20 questions, 30 minute timer, 75% to pass — just like the real test</p>
            </div>
            <div className="flex flex-col items-center text-center">
              <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-brand-red-light text-brand-red">
              ...
              <p className="text-[#424242]">See your improvement over time and identify weak topics</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="topics" className="bg-[#F7F7F7] py-16 md:py-24">
        <div className="container mx-auto px-4">
          <div className="mb-12 text-center">
            <h2 className="mb-3 text-3xl font-bold text-brand-navy md:text-4xl">Practice by Chapter</h2>
            <p className="text-lg text-[#4A4A4A]">Complete all chapters to be fully prepared</p>
            <p className="mt-4">
              <Link href="/study/complete-questions" className="text-base font-semibold text-brand-red underline underline-offset-2 hover:text-brand-red-dark">
          ...
          <div className="mx-auto grid max-w-6xl gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {LANDING_CHAPTERS.map((ch) => (
              <div key={ch.n} className="group flex flex-col rounded-[12px] border border-[#E0E0E0] bg-white p-5 shadow-[0_2px_8px_rgba(0,0,0,0.08)] transition-shadow duration-200 hover:shadow-[0_4px_16px_rgba(0,0,0,0.12)]">
                ...
                <div className="mt-auto flex items-center justify-between border-t border-[#E0E0E0] pt-4">
                  ...
                  <Link href="/signup" className="inline-flex items-center gap-1 text-sm font-semibold text-[#E8192C] hover:text-[#C41020]">
                    Start Practice
      ...
      <section id="success-stories" className="bg-[#F7F7F7] py-16 md:py-24">
        <div className="container mx-auto px-4">
          <h2 className="mb-12 text-center text-3xl font-bold text-brand-navy md:text-4xl">Success Stories</h2>
          <div className="mx-auto grid max-w-4xl gap-8 md:grid-cols-2">
            {TESTIMONIALS.map((t) => (
              <div key={t.name} className="rounded-[12px] border border-[#E0E0E0] bg-white p-8 shadow-[0_2px_8px_rgba(0,0,0,0.08)]">
        ...
        <div className="container mx-auto max-w-5xl px-4">
          ...
          <div className="grid gap-6 sm:grid-cols-3">
            {(
              ...
              ] satisfies ReadonlyArray<{ label: string; price: string; original: string; desc: string; popular?: boolean; best?: boolean }>
            ).map((plan) => (
              <div key={plan.label} className={['relative flex flex-col rounded-2xl border p-6', plan.popular ? 'border-brand-navy bg-brand-navy text-white shadow-lg' : 'border-gray-200 bg-white'].join(' ')}>
                {plan.popular && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                    <span className="rounded-full bg-brand-red px-3 py-0.5 text-xs font-bold uppercase tracking-wider text-white">Most Popular</span>
                  </div>
                )}
                {plan.best && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                    <span className="rounded-full bg-green-600 px-3 py-0.5 text-xs font-bold uppercase tracking-wider text-white">Best Value</span>
                  </div>
                )}
                <p className={['text-xs font-bold uppercase tracking-wider', plan.popular ? 'text-white/60' : 'text-gray-400'].join(' ')}>{plan.label}</p>
                <p className={['mt-1 text-sm', plan.popular ? 'text-white/70' : 'text-gray-500'].join(' ')}>{plan.desc}</p>
                <div className="mt-4 flex items-baseline gap-2">
                  <span className={['text-3xl font-extrabold', plan.popular ? 'text-white' : 'text-gray-900'].join(' ')}>{plan.price}</span>
                  <span className={['text-sm line-through', plan.popular ? 'text-white/40' : 'text-gray-300'].join(' ')}>{plan.original}</span>
        ...
        <section className="border-t border-[#E0E0E0] bg-white py-16 md:py-24">
          <div className="container mx-auto px-4">
            <div className="mb-10 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
              ...
              <Link href="/blog" className="text-base font-semibold text-brand-red hover:text-brand-red-dark">View All Posts →</Link>
            </div>
              ...
              <div className="flex flex-col md:flex-row md:items-stretch">
                <div className="relative aspect-video w-full shrink-0 overflow-hidden rounded-t-xl md:w-[60%] md:rounded-l-xl md:rounded-tr-none">
                  {featuredPost.cover_image ? (
                    <Image src={featuredPost.cover_image} alt={featuredPost.title} fill className="object-cover" sizes="(max-width: 768px) 100vw, 60vw" />
                  ) : (
                ...
                <div className="flex w-full flex-col justify-center gap-3 p-6 md:w-[40%] md:p-8">
                  <span className="inline-flex w-fit rounded-full bg-brand-red px-2.5 py-0.5 text-xs font-semibold text-white">Featured</span>
                  <p className="text-sm text-gray-500">{formatBlogDate(featuredPost.published_at)}</p>
                  <h3 className="line-clamp-2 text-2xl font-bold text-brand-navy">{featuredPost.title}</h3>
                  {featuredPost.excerpt ? <p className="line-clamp-3 text-gray-600">{featuredPost.excerpt}</p> : null}
                  <div className="pt-1">
                    <Button variant="outline" className="rounded-full border-2 border-brand-red bg-transparent text-brand-red hover:bg-brand-red-light hover:text-brand-red-dark" asChild>
            ...
            {recent.length > 0 ? (
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {recent.map((post) => (
                  <article key={post.id} className="flex flex-col overflow-hidden rounded-[12px] border border-[#E0E0E0] bg-white shadow-card">
                    <div className="relative aspect-video w-full shrink-0 overflow-hidden rounded-t-xl">
                      {post.cover_image ? (
                        <Image src={post.cover_image} alt={post.title} fill className="object-cover" sizes="(max-width: 1024px) 50vw, 33vw" />
                      ) : (
                    ...
                    <div className="flex flex-1 flex-col gap-2 p-5">
                      ...
                      {post.excerpt ? <p className="line-clamp-2 flex-1 text-sm text-gray-600">{post.excerpt}</p> : null}
                      <Link href={`/blog/${post.slug}`} className="text-sm font-semibold text-brand-red hover:text-brand-red-dark">Read More →</Link>
                    </div>
                  </article>
                ))}
              </div>
      ...
      <section className="relative overflow-hidden bg-[#E8192C] py-16 md:py-24">
        <div className="container relative mx-auto px-4 text-center">
          <h2 className="mb-4 text-3xl font-extrabold tracking-tight text-white md:text-4xl">Ready to Pass Your Citizenship Test?</h2>
          <p className="mx-auto mb-8 max-w-2xl text-lg text-white/90">Join thousands of Canadians who prepared with CitizenReady</p>
          <Button size="lg" className="h-12 rounded-full bg-white px-8 text-base font-semibold text-[#E8192C] hover:bg-[#FFF0F0]" asChild>
