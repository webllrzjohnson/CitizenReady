...
export default async function ExamResultsPage({ params }: ExamResultsPageProps) {
  ...
  const sessionRows = await sql`
    SELECT id, score, total_q, completed_at, created_at, question_ids
    FROM public.quiz_sessions
    WHERE id = ${sessionId}::uuid AND user_id = ${session.id}::uuid
    LIMIT 1
  `
  if (sessionRows.length === 0) redirect('/dashboard/mock-exam')

  const examSession = sessionRows[0]
  if (!examSession.completed_at) redirect(`/dashboard/mock-exam/${sessionId}`)
  ...
  const attemptsData = await sql`
    SELECT qa.id, qa.question_id, qa.user_answer, qa.is_correct,
           q.question_text, q.options, q.correct_answers, q.explanation, q.type
    FROM public.question_attempts qa
    JOIN public.questions q ON q.id = qa.question_id
    WHERE qa.session_id = ${sessionId}::uuid
    ORDER BY qa.created_at ASC
  `

  const attemptsMap = new Map(attemptsData.map((a: any) => [a.question_id, a]))
  const orderedAttempts = questionIds
    .map((qid: string) => attemptsMap.get(qid))
    .filter(Boolean)

  const score = examSession.score ?? 0
  const total = examSession.total_q
  const percentage = Math.round((score / total) * 100)
  const passed = score >= 15

  const timeTaken = examSession.completed_at && examSession.created_at
    ? Math.round((new Date(examSession.completed_at).getTime() - new Date(examSession.created_at).getTime()) / 1000 / 60)
    : 0

  const completedDate = examSession.completed_at
    ? new Date(examSession.completed_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
    : ''
    ...
    <div className="mx-auto max-w-5xl space-y-8 py-8">
        ...
        <div className="relative space-y-3">
          <div className="flex justify-center">
            <div className="flex h-20 w-20 items-center justify-center rounded-full bg-white/15 ring-4 ring-white/20">
              {passed ? <Trophy className="h-10 w-10 text-white" aria-hidden /> : <AlertCircle className="h-10 w-10 text-white" aria-hidden />}
            </div>
          </div>
          ...
          <p className="text-base text-white/80">{passed ? 'You passed the mock citizenship exam!' : 'You need 15/20 (75%) to pass. Keep practicing!'}</p>
        </div>
      </div>

      <div className="grid gap-4 sm:grid-cols-3">
        <div className="flex items-center gap-4 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className={cn('flex h-12 w-12 shrink-0 items-center justify-center rounded-xl', passed ? 'bg-green-50' : 'bg-red-50')}>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div className="border-b border-gray-100 px-6 py-4">
          ...
          <p className="mt-0.5 text-sm text-gray-500">Your answers and the correct responses with explanations</p>
        </div>
        <div className="divide-y divide-gray-100 px-6">
          {orderedAttempts.map((attempt: any, idx: number) => {
            if (!attempt) return null
            const userAnswer = typeof attempt.user_answer === 'string' ? JSON.parse(attempt.user_answer) : attempt.user_answer
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 space-y-3">
                    <div className="flex items-start justify-between gap-4">
                      <h3 className="text-base font-semibold leading-snug text-gray-900">{attempt.question_text}</h3>
                      {isCorrect ? (
                    ...
                    <div className="space-y-2">
                      {options?.map((option: any) => {
                        ...
                        return (
                          <QuizOptionRow key={option.key} optionKey={option.key} text={option.text} visual={visual} interactive={false}
                            suffix={<>{isUserAnswer && <span className="ml-2 text-xs font-semibold text-brand-navy">Your answer</span>}{isCorrectAnswer && <span className="ml-2 text-xs font-semibold text-green-700">Correct</span>}</>}
                          />
                        )
                      })}
                    </div>
